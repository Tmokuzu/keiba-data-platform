...
def train_lightgbm_place(
    ...
    try:
        ...
        raise RuntimeError("lightgbm is required. Run `uv sync` after updating requirements.") from exc
    ...
    save_artifact(output_model_path or MODELS_DIR / "lgbm_place_model.pkl", artifact)
    save_json(output_metrics_path or MODELS_DIR / "lgbm_place_metrics.json", metrics)
    logger.info("LightGBM place model metrics: %s", metrics)
    return artifact
